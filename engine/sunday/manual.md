...
## MCP 通道（熱路徑的 typed 工具版）

本手冊的**熱路徑端點另有 typed MCP 工具版**（MCP server `sunday`，工具名 `mcp__sunday__*`）：
唯讀 `markets_list / market_get / klines / indicators / funding / indices / positions / balance /
pnl_drawdown / open_orders / order_history / trades / protection_status`，寫入 `place_order /
close_position / set_protection / cancel_order / cancel_all_orders / set_leverage_margin /
alert_set / alerts_manage`。參數與輸出見**工具自帶的 schema**（此處不重複）；寫入工具由 schema
強制 `agent`（= `X-Agent` 歸責）與 `take_profit` / `stop_loss` / `memo`。

**本手冊仍是完整合約與降級通道的權威文件**：MCP 工具不可用（tool error / server 不在）時照本手冊
用 `http_request` 直打 `:7777`；journal / memory / reports 等長尾端點只有 HTTP 版。本手冊全文
也可經 MCP resource `sunday://manual` 讀取。
...
# 技術指標（set=rsi,ema,sma,macd,bollinger,adx,atr）。K 線短暫快取（隨 interval 調整、
# 最長 5 分鐘）；上游故障時回最後一次成功值並帶 `stale: true` + `stale_age_s`——拿到 stale
# 表示行情源暫時不穩，數值仍可用於判讀，但別當即時價；`as_of` 永遠是資料的真實 bar 時間。
