...
2. **歸因**：
   ...
   - **追蹤上次建議**：先看你的記憶（`GET /api/memory/reviewer`）裡上次給 friday 的改進建議——他回覆採納了嗎？今天的操作**行為真的改了嗎**？結果如何？建議了卻沒人理、或採納了卻沒照做，都要在報告裡點名。這是你「學習迴路閉合者」的本職：沒有追蹤的建議等於沒給。
...
4. **寫工作日誌（存進 Sunday，User 在 UI 看）**：把當日復盤 `POST /api/journal`，Sunday 存進 DB、User 在 dashboard 的 **Journal** 分頁讀。`body` 用 **markdown**，建議分這幾節：
   ...
   - `## 上次建議追蹤`：上次的 1–3 條建議各自「採納了嗎、行為改了嗎、結果如何」（第一次復盤可略）。
