...
決策、也不做硬性風控。evva swarm 的 leader——friday——是指揮官 / PM：他整合研究做交易決策、
寫成 order ticket；專職 worker——trader——用通用 `http_request` 呼叫 `POST /api/perp/order`
執行；其餘六個 worker 餵情報、踩煞車、復盤。** 風險紀律不在程式裡，在 friday 與 risk-monitor
談定的共識 + trader 的 pre-flight 核對 + 每筆單強制掛交易所原生 TP/SL。
...
2. **Sunday → swarm** — webhook `POST :8888/api/swarm/sunday/event`（RP-9），payload
   ...
   `position_pnl` 的收件人由 `MONITOR_WEBHOOK_TO` 決定（預設 `leader`，可設 `trader`）。
...
## 2. ⭐ 誰執行下單？（決策與執行分離——本文最重要的一節）

**答案：friday（指揮官）決定，trader（執行台）下單。** friday 整合研究寫成 order ticket
（標的/方向/大小/槓桿/TP/SL/理由/standing rules，走 task 或急件訊息）；trader pre-flight 核對
風控共識與市場限額後呼叫 `POST /api/perp/order`，Sunday 原樣轉發到幣安 testnet。Sunday
**不再**有 thesis/conviction 翻譯層、不再有確定性風控熔斷——milestone-6 把那整套移除了。

| 層級 | 誰 | 做什麼 |
| --- | --- | --- |
| **決策** | **friday（LLM）** | 整合隊友判讀 → 寫 order ticket（含 TP/SL 與理由）→ `task_assign` 給 trader → 驗收 |
| **執行** | **trader（LLM）** | pre-flight（共識/限額/餘額）→ `POST /api/perp/order`·`/close`、撤單、調 TP/SL → 驗證成交與保護腿 → 回報；依 standing rules 管在倉部位 |
...
1. **鐵則：每筆開倉必帶 `take_profit` + `stop_loss`**（ticket 必填欄位 + trader 拒收殘缺
   ticket，無例外）。
2. **friday ↔ risk-monitor 共識**：單筆上限 / 槓桿 / 總曝險 / 回撤上限，談定後寫進
   `PUT /api/memory/friday`；trader 每張 ticket pre-flight 對照、risk-monitor 每小時巡檢，
   越線就警告。
3. **全程 testnet 假錢** + `permission_mode: bypass`（無人值守 7×24，operator 已確認）。
4. **緊急越權**：trader 失聯且事態緊急時 friday 可直接 `POST /api/perp/close` 砍倉，事後報備
   ——消防通道，不是日常通道。

> ⚠️ 注意這個信任模型的邊界：「只有 trader 例行下單」是 **prompt 紀律**，不是技術強制——Sunday
> API 免 token，任何 worker 技術上都打得到 `/api/perp/*`。各 agent 的 system prompt 明文劃界
> （研究員「不下單」、friday「例行走 trader」），而 testnet 假錢是這個實驗可以接受該邊界的原因。
...
| 角色 | 喚醒來源 | 做什麼 | **不做** |
| --- | --- | --- | --- |
| **friday**（leader） | webhook（`price_alert`，預設也收 `position_pnl`）/ 隊友 `send_message` / User / **30m** cron 安全網 | **指揮官 / PM**：醒來先 `GET /api/memory/friday` 回顧共識與持倉理由 → 對帳 → 決策寫成 order ticket 派 trader、派研究課題、`task_verify` 驗收、`schedule_set`/`alarm_set` 調度、閉環回覆 → 收工前 `PUT /api/memory/friday` 寫回；重大事件 `POST /api/reports` 通報 User | 不做例行執行（緊急越權除外）；不必每次醒來都動作（stand down 合法） |
| **trader** | friday 的 ticket（task/急件訊息）/ **每小時 :15·:45** 保護腿巡檢 / `position_pnl` webhook（設 `MONITOR_WEBHOOK_TO=trader` 時直達） | **執行台**：pre-flight 核對共識與限額 → 下單（必帶 TP/SL + memo）→ 驗證成交與保護腿 → 回報；依 standing rules 管在倉部位、撤孤兒掛單、對帳 | 不決定方向；殘缺 ticket（缺 SL）退回不執行 |
...
- **兩種事件**（`events.py`）：
  - `position_pnl` — 持倉 ROI 每跨一個 5% bucket（`MONITOR_STEP_PCT`）通報一次；喚醒誰由
    `MONITOR_WEBHOOK_TO` 決定（預設 `leader` → friday triage；設 `trader` 直達執行台，由他
    照 standing rules 處理、例外才上報 friday）。
...
## 6. 一輪協作走過的例子（webhook 喚醒 → 團隊研究 → ticket → 執行 → 復盤）

1. **開倉**：前一輪 friday 決定做多 SOLUSDT，開 ticket（task）給 trader：5×、isolated、
   TP 180 / SL 140、理由、standing rule「+10% ROI → SL 上移到成本」；trader pre-flight 過
   → `POST /api/perp/order` → 驗證成交與保護腿 → 回報；friday `task_verify` 結案，並
   `POST /api/alerts` 在 150 設了加碼觀察價。
...
5. **friday 裁決**：採納 → 急件 ticket 給 trader「SOL SL 上移到成本」→ trader 撤舊掛新、驗
   `protection` → 回報完成；friday 回 analyst-flow「採納，理由 X」→ `PUT /api/memory/friday`
   更新持倉理由 → 收工。
