# query-sunday 唯讀查 Sunday 的成交與績效（reviewer 復盤用）
...
1. **寫工作日誌**：`POST /api/journal` 存進 Sunday DB（User 在 dashboard 的 Journal 分頁看）。`body` 用 markdown，建議分節：當日操作 / 盈虧歸因 / 做對做錯 / 改進建議。
   ```jsonc
