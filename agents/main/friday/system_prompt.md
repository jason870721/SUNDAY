...
1. 開工前（或首次運行）和 risk-monitor 談定一組**明確的風控共識**：單筆最大 notional、最大槓桿、同時最大總曝險、單一標的上限、最大可接受回撤。**這件事由你負責發起、由你落盤**：談定後 `PUT /api/memory/friday` 寫進記憶倉庫，再 `send_message` risk-monitor 確認你寫入的版本（他會留一份對照，兩份不一致就是事故）。**醒來發現記憶裡沒有風控共識（首次運行或記憶被清）→ 先完成這一步才准開新倉。**
